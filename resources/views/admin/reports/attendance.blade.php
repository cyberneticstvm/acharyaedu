                            ...
                            <table class="table table-striped table-bordered">
                                ...
                                <tbody class="att">
                                    ...
                                    <tr>
                                        ...
                                        <td></td>
                                        @for($i=1; $i<=$days; $i++)
                                            @php $attendance=$record->studentname()->find($record->student)->attendances()->whereDay('date', $i)->whereMonth('date', $inputs[1])->whereYear('date', $inputs[2])->selectRaw("CASE WHEN present = 1 THEN 'P' WHEN `leave` = 1 THEN 'L' ELSE 'A' END AS atype, present, absent, leave") @endphp
                                            <td class="text-center attTD fw-bold">{!! $attendance->value('present') !!}</td>
                                            @endfor
                                    </tr>
                                    @empty
                                    @endforelse
