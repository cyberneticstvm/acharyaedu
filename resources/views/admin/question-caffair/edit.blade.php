    ...
    <div class="col-xl-12 col-lg-12 col-md-12">
        <div class="card">
            <div class="card-header pb-0 text-left bg-transparent">
                <h3 class="font-weight-bolder text-primary text-gradient">Update Current Affairs Question</h3>
            </div>
            <div class="card-body">
                <form role="form" method="post" action="{{ route('caffairquestion.update', $question->id) }}">
                    @csrf
                    @method("PUT")
                    <div class="row">
                        <div class="col-md-3">
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" name="month">
                                    @forelse($months as $key => $month)
                                        <option value="{{ $month->id }}" {{ ($month->id == $question->month) ? 'selected' : '' }}>{{ $month->name }}</option>
                                    @empty
                                    @endforelse
                                </select>                                  
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" name="year">
                                    @for($i=2020; $i<=date('Y'); $i++)
                                        <option value="{{ $i }}" {{ ($i == $question->year) ? 'selected' : ''}}>{{ $i }}</option>
                                    @endfor
                                </select>                                  
                            </div>
                            @error('year')
