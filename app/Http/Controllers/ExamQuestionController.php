...
class ExamQuestionController extends Controller
{
    ...
    public function create(Request $request, $id)
    {
        ...
        $input = $request->all();        
        $questions = Question::where('subject_id', $request->subject_id)->where('level_id', $request->level_id)->where('topic_id', $request->topic_id)->where('status', 1)->inRandomOrder()->limit($request->number_of_questions)->get();
