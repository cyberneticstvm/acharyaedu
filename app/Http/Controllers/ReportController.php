...
class ReportController extends Controller
{
    ...
    public function fetchattendance(Request $request)
    {
        ...
        //for ($i = 1; $i <= $days; $i++):
        $records = $record->studentname()->find($record->student)->attendances()->whereDay('date', '26')->whereMonth('date', '9')->whereYear('date', '2024')->selectRaw("CASE WHEN present = 1 THEN 'P' WHEN `leave` = 1 THEN 'L' ELSE 'A' END AS atype, present, absent, leave")->first();
        dd($records);
        //endfor;
        die;
        return view('admin.reports.attendance', compact('records', 'batches', 'months', 'years', 'inputs', 'days'));
