...
class ReportController extends Controller
{
    ...
    public function fetchDailyClosing(Request $request)
    {
        ...
        $students = Student::whereBetween('created_at', [Carbon::parse($request->from_date)->startOfDay(), Carbon::parse($request->to_date)->endOfDay()])->where('admission_fee_advance', '>', 0)->get();
    ...
    public function fetchattendance(Request $request)
    {
        ...
        //for ($i = 1; $i <= $days; $i++):
        $records = $record->studentname()->find($record->student)->attendances()->whereDay('date', '26')->whereMonth('date', '9')->whereYear('date', '2024')->selectRaw("`present`, `student`, `batch`, `date`, `absent`, `leave`, CASE WHEN present = 1 THEN 'P' WHEN `leave` = 1 THEN 'L' ELSE 'A' END AS atype")->first();
        dd($records);
        //endfor;
        die;
        return view('admin.reports.attendance', compact('records', 'batches', 'months', 'years', 'inputs', 'days'));
